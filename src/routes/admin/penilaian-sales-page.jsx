...
const PenilaianPage = () => {
    ...
    const {
        ...
        queryKey: ["get-all-penilaian"],
        queryFn: ({ pageParam }) => fetchAllPenilaian(pageParam),
        initialPageParam: 1,
    ...
    async function fetchAllPenilaian(pageParam) {
        // if (role !== "ADMIN") return [];
        const response = await axios.get("http://localhost:8082/sales/penilaiansales?page=1&limit=17", {
            params: {
                page: pageParam,
            },
        });
            ...
            {status === "pending" ? (
                ...
                <div className="w-full h-full overflow-y-auto space-y-2 pb-20">
                    {data.pages.map((group, i) => (
                        <PenilaianList key={i} data={group.content} />
                    ))}
...
const PenilaianList = ({ data }) => {
    return (
        <>
            {data.length < 1 ? (
                <div className="w-full h-full flex justify-center items-center">
                    <h1 className="text-lg font-semibold">Penilaian Kosong</h1>
                </div>
            ) : (
                <Fragment>
                    {data?.map((elem) => (
                        <PenilaianCard key={elem.idsales} data={elem} />
                    ))}
                </Fragment>
            )}
        </>
